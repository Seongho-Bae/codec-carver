## 2026-08-25 - [Optimize SQLite WAL PRAGMA Execution]
**Learning:** SQLite's `PRAGMA journal_mode=WAL` is persistent per database file. Executing it redundantly on every new short-lived connection incurs unnecessary performance overhead.
**Action:** Execute `PRAGMA journal_mode=WAL` exactly once during database initialization alongside the schema (e.g., via `conn.executescript("PRAGMA journal_mode=WAL;
" + _SCHEMA)`) instead of on every connection.
