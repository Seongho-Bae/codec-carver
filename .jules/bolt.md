## 2026-08-27 - [Optimize SQLite PRAGMA journal_mode=WAL execution]
**Learning:** SQLite PRAGMA journal_mode=WAL is persistent per database file. Executing it on every short-lived connection causes unnecessary I/O overhead.
**Action:** Execute PRAGMA journal_mode=WAL once during initialization (e.g. via conn.executescript() alongside the schema) instead of on every connection.
