## 2024-06-01 - Avoid executing PRAGMA journal_mode=WAL on every connect
**Learning:** SQLite's `PRAGMA journal_mode=WAL` is persistent per database file. Executing it on every short-lived connection is redundant and adds latency.
**Action:** Execute persistent PRAGMAs like WAL once during database initialization (e.g., alongside the schema creation using `conn.executescript`) instead of on every connection.
