## 2024-05-18 - Optimize SQLite WAL PRAGMA for Short-Lived Connections
**Learning:** SQLite `PRAGMA journal_mode=WAL` is persistent per database file. Executing it on every short-lived connection adds unnecessary overhead.
**Action:** Execute `PRAGMA journal_mode=WAL` once during database initialization (e.g. via `conn.executescript()`) rather than redundantly on every connection.
