## 2024-08-23 - Explicit required field validation state
**Learning:** When inputs are cleared, implicitly removing the invalid state and setting preview text to empty fails to convey the required nature of the field to screen readers and sighted users.
**Action:** Always provide explicit inline validation feedback (e.g., "This field is required.") and set `aria-invalid="true"` when a required field becomes empty, rather than just clearing previous errors.
