...
## 2026-08-04 - 인라인 폼 유효성 검사 최대값(Max) 피드백 추가
**학습:** 파일 업로드 폼에서 크기 제한(max) 속성만 제공하면 서버에서 거부될 때까지 사용자가 문제를 알 수 없거나 네이티브 팝업만 보게 됩니다. 대상 바이트와 같이 최댓값이 있는 필드에 JS 기반 실시간 미리보기 및 커스텀 에러 피드백을 추가하면 제출 전 오류를 방지할 수 있습니다.
**실행:** 최댓값이 정의된 숫자 입력 필드에 HTML5 `max` 속성을 추가하고, 실시간 `input` 이벤트 리스너에서 최댓값 초과 여부를 확인하여 `setCustomValidity` 및 `aria-invalid`로 시각적/보조기기 피드백을 즉시 제공해야 합니다.
