...
## 2024-08-05 - HTML5 max 제약과 JavaScript 유효성 검사 동기화
**학습:** 숫자 입력 필드에 HTML5 `max` 제약을 추가하더라도, 프로그램에 의한 입력이나 붙여넣기로는 브라우저의 UI 제약을 우회할 수 있습니다. 따라서 접근성 있는 오류 상태를 올바르게 전달하려면 일치하는 JavaScript 클라이언트 측 유효성 검사(`setCustomValidity` 및 `aria-invalid`)가 필수적입니다.
**실행:** 숫자 입력에 HTML5 제약 조건(`max`, `min` 등)을 추가할 때, 스크린 리더와 시각적 피드백 모두에서 오류 상태를 신뢰할 수 있게 노출할 수 있도록 항상 JavaScript 유효성 검사와 동기화하여 구현하십시오.
