...
## 2024-05-18 - 숫자 입력의 HTML5 제약 조건과 접근성 연동
**학습:** HTML5의 `max` 제약 조건을 추가할 때, 브라우저의 기본 UI를 우회하여 프로그래밍 방식으로 입력되거나 붙여넣기가 발생할 경우 스크린 리더 등에서 에러 상태를 제대로 인식하지 못할 수 있습니다.
**실행:** `max` 등 제약 조건을 추가할 때는 반드시 JavaScript의 `setCustomValidity` 및 `aria-invalid` 속성 업데이트를 명시적으로 구현하여 접근성 에러 상태가 올바르게 표출되도록 해야 합니다.
