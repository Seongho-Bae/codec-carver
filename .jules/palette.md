## 2024-08-05 - Required Form Input Empty State Feedback
**Learning:** When client-side validation logic handles an empty input state (e.g. user clearing a required field), simply clearing the custom validity and aria-invalid attribute silently removes feedback and leaves the screen reader without context. Custom inline styles should not be used for error messages.
**Action:** Always explicitly set an inline error message like 'This field is required.' and set `aria-invalid="true"` when a required field is emptied, to provide clear feedback to all users. Apply existing classes and clear inline styles via `element.style.color = '';` instead of setting new inline styles.
