...
## 2024-10-24 - HTML5 max 속성과 클라이언트 측 검증을 통한 사용성 개선
**학습:** 숫자 입력 필드에 서버 측 최대값 제한이 있음에도 HTML5 max 속성이나 클라이언트 측 JS 검증이 누락되면, 사용자는 큰 값을 입력하고 제출한 뒤에야 오류를 발견하게 되어 UX와 접근성이 저하됩니다.
**실행:** 백엔드에 상한선이 있는 숫자 입력 필드에는 항상 HTML5 max 속성을 추가하고, 범위를 초과할 경우 setCustomValidity 및 aria-invalid를 사용해 즉각적인 인라인 시각적/보조기기 오류 피드백을 제공해야 합니다.
